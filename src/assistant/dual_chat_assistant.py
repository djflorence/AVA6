"""
Enhanced chat assistant implementation using DualMemoryManager.

This module provides a chat assistant that uses separate memory collections
for Ava's memories and David's information, preventing data overwriting and
...
class DualChatAssistant:
    """
    Enhanced chat assistant using DualMemoryManager.

    This class integrates various components:
    - LLM for generating responses
    - Dual memory system for separate Ava and David memories
    ...
    def __init__(self, config: Dict[str, Any]):
        """
        Initialize the dual chat assistant.
        ...
        logger.info("Dual chat assistant initialized")
    ...
    def _init_memory(self) -> None:
        ...
        # Initialize the dual memory manager
        chroma_dir = self.config.get("chroma_db_directory", "./src/data/chroma")
        
        # Check if a mock memory manager is provided for testing
        if "_mock_memory_manager" in self.config:
            logger.info("Using mock memory manager for testing")
            self.memory_manager = self.config["_mock_memory_manager"]
        else:
            # Initialize the dual memory manager
            self.memory_manager = DualMemoryManager(
                chroma_dir=chroma_dir,
                window_size=10,
                conversation_id=self.config.get("conversation_id", f"conversation_{datetime.now().strftime('%Y%m%d_%H%M%S')}"),
                batch_size=100,
    ...
    def _get_chat_history(self) -> List[Union[HumanMessage, AIMessage, SystemMessage]]:
        ...
        if self.memory_manager:
            ...
            recent_interactions = self.memory_manager.ava_memory.get_recent_interactions()
